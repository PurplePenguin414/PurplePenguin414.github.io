...
class StringQueue 
{
    ...
    StringQueue(int capacity) 
    {
     ...
     /**
     * Copy Constructor.  Clones existing object exactly.
     *
     * @param capacity The capacity of the queue.
     */
    StringQueue(StringQueue existingQueue)
    {
        qArray = new String[existingQueue.qArray.length];
        front = existingQueue.front;
        rear = existingQueue.rear;
        size = existingQueue.size;
        
        for (int i = 0; i < qArray.length; i++)
            qArray[i] = existingQueue.qArray[i];
    }
    ...
    public boolean isFull() 
    {
     ...
     /**
     * Method requires true if both queues have exactly the same number of
     * elements and in the same sequence.
     *
     * @return Boolean result for equivalence defined in description.
     */
    public boolean equals(StringQueue queue2) 
    {  
        ...
        int pos0 = this.front;
        int pos1 = queue2.front;
        while (pos0 != this.rear)
        {
            // Check current queue element
            if (this.qArray[pos0] != queue2.qArray[pos1])
                return false;
            
            // Advance both queue positions
            pos0++;
            pos1++;
            if (pos0 == this.qArray.length)
                pos0 = 0;
            if (pos1 == queue2.qArray.length)
                pos1 = 0;       
        }

        return true;    // Only remaining outcome is queue equivalence 
    }
    /**
