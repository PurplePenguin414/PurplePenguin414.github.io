...
class LinkedList<ItemType>
{
    ...
    // --------------------------------------------------------------
    // Method searches for existence of target in list using a recursive
    // approach.
    // Param:  target.  Search target.
    // Return:  boolean.  Found target or did not find target
    public boolean containsRecurse(ItemType target)
    {
	return searchFor(first, target);     
    }
    
    // Private recursive method to perform a search for a given list item.
    private boolean searchFor(Node thisNode, ItemType target)
    {
        // Base case
        if (thisNode == null)
            return false;
        ...
        if (thisNode.value.equals(target))
            return true;

        // Recurse for remaining list
        return searchFor(thisNode.next, target);
    }
}